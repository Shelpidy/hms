  ...
  try {
    ...
    const appointmentsWithDetails = await Promise.all(
      appointments.map(async (appointment: any) => {
        ...
        let doctorRoomId = await Room.findOne({
          where: {
            [Op.and]: [
              {
                [Op.or]: [
        ...
        let patientRoomId = await Room.findOne({
          where: {
            [Op.and]: [
              {
                [Op.or]: [
        ...
        return {
          doctor: {
            ...
            specialization,
            user: doctorUser,
            roomId: doctorRoomId?.getDataValue("roomId"),
          },
          patient: {
            patient,
            bloodGroup,
            user: patientUser,
            roomId: patientRoomId?.getDataValue("roomId"),
          },
          appointment: {
